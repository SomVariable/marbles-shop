...
BaseFolder: TypeAlias = Literal["components", "pages", "modules"]
...
class ReducerFileCreator(FileCreator):
    """Element.tsx file creator"""
    def get_absolute_filename(self) -> Path:
...
class AskParams:
    ...
    def _ask_base_folder(self) -> BaseFolder:
        while True:
            match input("c — components, p — pages, m — modules: ").strip().lower():
                case "c": return "components"
                case "p": return "pages"
                case "m": return "modules"
                case _: print("Не понял, давай ещё раз.")
...
def main():
    ...
    if element.layer == "components":
        element_creator.register_file_creators(
        TSXFileCreator,
        CSSFileCreator,
        IndexFileCreator
    )
    elif element.layer == "pages":
        element_creator.register_file_creators(
        TSXFileCreator,
        CSSFileCreator,
        IndexFileCreator
    )
    elif element.layer == "modules":
        element_creator.register_file_creators(
        TSXFileCreator,
        CSSFileCreator,
        IndexFileCreator,
